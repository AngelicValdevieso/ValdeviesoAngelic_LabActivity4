package com.valdevieso.laboratoryactivity4;
...
public class MainActivity extends AppCompatActivity implements AdapterView.OnItemClickListener {
    ...
    ArrayList<Version> androidVersion = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int i = 0; i < names.length; i++) {
            androidVersion.add(new Version(cLogo[i], names[i], version[i], apiLvl[i], rDate[i], message[i]));

        }
        list = findViewById(R.id.lvAndroidVersions);
        AndroidAdapter adapter = new AndroidAdapter(this, R.layout.item, androidVersion);
